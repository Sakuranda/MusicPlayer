...
3. **下载双路线**：DASH 直连（view→cid→playurl→CDN 直链给 yt-dlp generic 下载，`Referer: https://www.bilibili.com/` 必需，否则 CDN 403）→ 失败回退 yt-dlp 网页路线。数据中心 IP 会吃 412 / "bad parameter or other API misuse"（限流）—— 已内置多级等待重试 + 批次结束后自动重试一轮失败项。
